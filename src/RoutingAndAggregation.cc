...
void (*sendAMessage) (unsigned short _interface, unsigned char* _msg);
...
//void handle_message_breakup(unsigned char* _msg, unsigned short inf)
//{
...
unsigned char* write_packet()
{
	...
	pkt[0] = outgoing_packet.message_type;
	pkt[1] = outgoing_packet.length;
	memcpy(&(pkt[2]), outgoing_packet.data, outgoing_packet.length);
	//incoming_packet.data = (unsigned char*)realloc(incoming_packet.data, incoming_packet.length);
	//memcpy(incoming_packet.data, &(pkt[2]), incoming_packet.length);
	//char lsb = pkt[2+incoming_packet.length];
	//char msb = pkt[2+incoming_packet.length+1];
	//incoming_packet.path_value = (signed short)pkt[2+incoming_packet.length];
	...
	// need to take care that this is the right byte to use to ref the 2 bytes
	// ie msb? lsb? etc...
	//signed short* x = (signed short*)(pkt+2+incoming_packet.length);
	//incoming_packet.path_value = *x;
	pkt[2+outgoing_packet.length] = (char)(outgoing_packet.path_value & 0xFF); // lsb goes first in buff;
	pkt[2+outgoing_packet.length+1] = (char)(outgoing_packet.path_value >> 8); // msb goes second in buff;
	if (pkt[0] == 0 && pkt[1] == 0 )
	{
	    pkt[1] = 0;
	}
 ...
 *
 */
 void traverse(trie *t, char* str, unsigned short inf, void process(State* s, unsigned char* _data, unsigned short _if))
 {
 		if ( t != NULL )
 		{
 			if ( t->keyelem )
 ...
 */
 ...
 void ucast_best_gradient(State* s, unsigned char* _data, unsigned short _if)
  {
 ...
 */
//int UcastAllBestGradients(StateNode* tree, unsigned short inf)
int UcastAllBestGradients(trie* t, unsigned short inf)
 {
 ...
 * Just about OK
 */
struct KDGradientNode* newKDGradientNode(char* fullyqualifiedname, int iName, int obtain, int deliver)
{
...
struct KDGradientNode* insertKDGradientNode1(char* fullyqualifiedname, int iName, int costType, int pCost, struct KDGradientNode* treeNode, int lev )
{
...
void reinforceDeliverGradient(char* fullyqualifiedname, unsigned short iName)
{
	rd->grTree = insertKDGradientNode1(fullyqualifiedname, iName, REINFORCE_DELIVER, 0, rd->grTree, 0);
}


void reinforceObtainGradient(char* fullyqualifiedname, unsigned short iName)
{
...
void setObtainGradient(char* fullyqualifiedname, int iName, int pCost)
{
	rd->grTree = insertKDGradientNode1(fullyqualifiedname, iName, OBTAIN, pCost, rd->grTree, 0);
}


void setDeliverGradient(char* fullyqualifiedname, int iName, int pCost)
{
...
void displayState(State* s, unsigned char* _data, unsigned short _if)
{
    int i = 0;
    switch(s->action)
    {
...
unsigned short excludedInterface;
void consider_sending_data(State* s, char* _buf, unsigned short _if)
{
...
void action_all_prefixes(trie *t, int i, int n, const char *str, char* buf, unsigned short _if, void process(State* s, char* _buf, unsigned short _if))
{
      trie *current = t;
...
void (*h[9]) (unsigned short _interface) =
{
handle_advert,
handle_interest,
handle_reinforce,
...
void setMessageCallBack(void (*_sendAMessage) (unsigned short _interface, unsigned char* _msg))
{
...
void handle_message(unsigned char* _msg, unsigned short inf)
{
	//memcpy(incoming_packet.packet_bytes, _msg, MESSAGE_SIZE);
	read_packet(_msg);
	//(*h[incoming_packet.the_message.message_type]) (inf);
...
void handle_collaboration(unsigned short _interface)
{
...
void handle_reinforce(unsigned short _interface)
{
	...
	if ( t && t->s->bestGradientToObtain )
	{
		...
		unsigned short interface = t->s->bestGradientToObtain->key2->iName;
...
void handle_reinforce_interest(unsigned short _interface)
{
	...
	if ( t && t->s->bestGradientToDeliver )
	{
		...
		// find the next interface to reinforce
		unsigned short interface = t->s->bestGradientToDeliver->key2->iName;
...
void handle_reinforce_collaboration(unsigned short _interface)
{
    reinforceObtainGradient((char*)incoming_packet.data, _interface);
...
(char* fullyqualifiedname, unsigned short iName)
 ...
 */
// probably not thread safe
void start_reinforce(char* fullyqualifiedname, unsigned short _if)
{
 ...
 */
// probably not thread safe
void start_reinforce_interest(char* fullyqualifiedname, unsigned short _if)
{
 ...
 * only treat as arrived from SELF?
 */
void start_reinforce_collaboration(char* fullyqualifiedname, unsigned short _if)
{
    reinforceObtainGradient(fullyqualifiedname, SELF_INTERFACE);
    //StateNode* sn = FindStateNode(rd->stateTree, specific_data_value._dataname_struct1.the_dataname);
    trie* t = trie_add(rd->top_state, fullyqualifiedname, STATE);
 ...
 */
void consider_reinforce_interest(State* s, char* _buf, unsigned short _if)
{
 ...
 * if the prefix has a best deliver gradient but no selected (reinforced) delivery interface
 */
void consider_reinforce_collaberation(State* s, char* _buf, unsigned short _if)
{
    if ( s->bestGradientToDeliver && !s->deliveryInterfaces )
    {
        // second parameter no longer used, pos pass s in future to save
...
int numSinks;
void count_sink(State* s, char* _buf, unsigned short _if)
{
    if ( s->action == SINK_ACTION )
    {
        numSinks++;
...
void processState(State* s, unsigned char* _data, unsigned short _if)
{
	int temp = dataRate;
...
void new_one(trie *t, const char *str, unsigned short _if)
{
   const int n = strlen(str);
   //f(t, 0, n, str, t);
